...
*Document de production — v2.0 (2026-08-09).*
...
>
> **Ce qui disparaît :** le minage exploratoire souterrain (remplacé par des **filons de surface à récolter**, façon Elin), la construction voxel libre (remplacée par une **construction cadrée** en empreintes de bâtiments), le volume souterrain (remplacé par des **donjons en étages discrets**).
...
> |---|---|---|
> | **0** | **Prototype de combat isolé** (document séparé) | *Le combat est-il bon ?* Rien ne démarre avant un oui. |
> | **1** | Projet réel : combat rapatrié + pipeline paperdoll minimal (une silhouette, quelques pièces d'équipement visibles) | un combat propre dans le vrai projet |
> | **2** | **Génération de donjon** (salles et connecteurs sur grille, étages, hauteur) | un espace clos à explorer |
> | **3** | **Loot** : affixes générateurs, gemmes, sertissures, rareté par profondeur | une raison de descendre |
> | **4** | **Progression** : usage, potentiel, niveaux d'élément et de construction | une raison de recommencer |
...
> | **6** | **Matériaux (~30, justifiés un par un)** + craft compositionnel | fabriquer ce qu'on n'a pas looté |
> | **7** | **Camp de base** : une seule cellule, stations, coffres, repos | un point d'ancrage entre deux expéditions |
> | **8** | **Génération du monde** : grille continue, hauteur, biomes, cellules, carte du monde, voyage rapide | un monde à parcourir entre les donjons |
> | **9** | **PNJ et villages** : dialogue, relations, information par paliers, boutiques | un monde habité |
> | **10** | **Royaumes, lois, économie, claims** | l'endgame de territoire |
> | **11** | **Coop** (temporalités parallèles, 5.0) | dernier chantier, jamais avant un solo bon |
>
...
>
> **1. Une partie solo EST une partie multijoueur hébergée dont la porte est fermée.** Ce n'est pas du contenu réseau — c'est une discipline d'architecture, et elle ne coûte presque rien si elle est respectée dès le départ. Elle coûte une réécriture complète si elle est ajoutée après.
> - **Serveur autoritaire, même en solo** : toute la logique de jeu vit côté serveur ; le client envoie des *intentions* et affiche un *état*. En solo les deux tournent dans le même processus, **jamais dans le même code**.
> - **Aucun système de gameplay ne lit l'input directement.** Le client produit « je veux attaquer cette tuile », le serveur décide.
> - **L'état du monde est sérialisable en permanence** — même exigence que la sauvegarde, payée une fois pour deux usages.
> - **Déterminisme** : génération seedée, résolution par ticks, aucun recours au delta de frame dans la logique.
> - **Rien ne suppose « un seul joueur »** : pas de singleton `Player`, pas de caméra qui pilote la logique, pas de pause globale.
> - **Les temporalités parallèles sont une notion du modèle dès le départ** (5.0) : une horloge du monde, une par combat, une par donjon. Écrire une horloge unique globale en solo garantit de tout réécrire plus tard.
>
> *Ce qui reste légitimement en étape 11 : le transport réseau, la découverte de parties, la latence, la reconnexion, l'interpolation. Cela s'ajoute sans rien casser — à condition que ce qui précède soit respecté.*
>
> **2. Une brique à la fois**, chacune avec un critère de sortie formulé AVANT de commencer.
>
> **3. Une seule langue au départ**, mais toutes les chaînes affichables passent par `tr()` dès le premier écran.
>
> **4. Tout le contenu est de la donnée** (Annexe B) : aucune valeur de gameplay codée en dur, validation des schémas au boot, rechargement à chaud.

---

## 1. Concept général

**Pitch :** Un roguelike tactique en monde infini généré procéduralement et totalement continu, en **vue isométrique sur grille**, où une carte du monde (case par case) sert de couche stratégique et de voyage rapide au-dessus d'un monde parcouru librement en temps réel — avec un **combat en action-time à ticks** structuré par les cinq éléments du **Wu Xing** et sa jauge de chaîne, une progression par l'usage à la Elona/Elin, et un endgame de construction de royaume.
...
|---|---|
| Dwarf Fortress | Densité systémique, monde simulé, jeu de données |
| Dofus | Vue isométrique, paperdoll billboard pixel art, grille tactique |
| Final Fantasy Tactics / Disgaea | Hauteur de terrain quantifiée, tactique de placement |
...
**Boucle macro :**
Exploration du monde continu (à pied en temps réel, ou via voyage rapide par la carte du monde) → points d'intérêt (donjons, villages, ressources) → exploration/combat/construction/récolte → retour à la base/claims.
...
- **Intérieur d'une cellule (génération de la grille)** : les mêmes familles de couches (affinées/dérivées) génèrent la **hauteur de chaque tuile** (3.6), le matériau de sol, les filons de surface et le contenu, en cohérence avec le biome déterminé à l'échelle macro.
...
### 3.2 Structure du monde : grille continue
...
Chaque tuile porte : une **hauteur entière** (0-20, voir 3.6), un **matériau de sol**, un **contenu** éventuel (mur, arbre, filon, meuble, empreinte de bâtiment) et un **occupant** éventuel (créature). Structure plate, lisible, sérialisable.

- **Continuité réelle** : on marche d'une cellule à l'autre sans rupture ; les chunks se génèrent devant et se déchargent derrière. La carte du monde reste un **résumé** du même champ de bruit (E.2) — le voyage rapide est un raccourci par-dessus un monde qui existe vraiment.
- **Pas de volume souterrain.** Le souterrain n'existe plus comme espace continu : il devient les **donjons**, grilles séparées en étages discrets reliés par des escaliers (3.5). Les ressources minérales se récoltent en **filons de surface** (façon Elin), pas en creusant des tunnels — le minage exploratoire est explicitement écarté.
...
- **Zones de coup dérivées du dénivelé** (5.1) : frapper d'en haut donne la tête, d'en bas les jambes.
- **Les fluides coulent** : l'eau remplit les creux, la lave descend. E.22 se simplifie en 2D + hauteur au lieu d'un volume.
- **Destruction** : on peut **abaisser ou élever une tuile** (tranchée, talus), **détruire un mur ou un bâtiment** (la tuile redevient sol), **abattre un arbre**. La destruction reste tactiquement lisible — effondrer le pont, ouvrir une brèche, inonder la tranchée — sans permettre le tunnel arbitraire.
...
## 4. Construction et artisanat

### 4.1 Construction cadrée (remplace la construction voxel libre)
...
- **Empreintes de bâtiments** : un bâtiment est une empreinte de tuiles + une hauteur de murs, posée depuis un catalogue ou dessinée tuile par tuile sur ses cases claim (3.3). La **détection de pièces** (E.5, qui pilote le logement des PNJ) devient triviale en 2D.
...
- **Aménagement** : murs, portes, sols, meubles (F.6), stations d'artisanat, pièges, palissades — tout se pose à la tuile.
- **Destruction** : voir 3.6. Elle vaut pour les bâtiments PNJ comme pour les siens.
...
### 5.0 Le temps : action-time à ticks
...
- **Architecture à ticks conservée** (Annexe D) : toute la simulation (IA, statuts, croissance, économie, faim) est pilotée par des ticks, jamais par le delta de frame. En exploration temps réel, l'horloge avance d'elle-même ; en combat, elle avance à l'action.
- **Vote** : la mécanique de vote ne subsiste que pour le **saut de nuit** (E.21).

### 5.1 Combat

**Combat tactique sur grille, en action-time** (5.0) :
...
- **Zones dérivées du DÉNIVELÉ**, jamais visées : attaquer une cible située **plus bas** donne la **tête** (×2.5), plus haut ne touche que les **jambes** (×0.8), à hauteur égale le **torse** (×1.0). La zone est une conséquence du positionnement — c'est ce qui rend la hauteur de terrain mécaniquement centrale (3.6).
...
- **Garde en posture** : prendre la garde coûte 2 ticks et dure jusqu'à la prochaine action. Réduit les dégâts **frontaux** de 80 %, coûte de l'endurance à l'impact. **La garde est frontale** — une attaque de flanc ou de dos l'ignore : c'est la pénalité naturelle de l'encerclement, et ce qui rend le placement payant.
- **Attaque lourde** : action distincte, coût ×2 en ticks, dégâts ×2.2, **brise la garde**. Elle est **visible par l'adversaire** pendant sa préparation (icône d'intention) — en tactique, le télégraphe est une **information d'interface**, pas une animation. C'est le gain décisif de la direction.
- **Endurance longue et lente** (A.6.1) : le combat se gère sur la durée. Attendre est une action qui rend de l'endurance.
- **Les jets de dés survivent hors combat** : le **jet de compétence universel** (1d20 + compétence/2 + stat/4 vs difficulté) reste la résolution de la lecture (A.7), du dressage, de la négociation, de la discrétion (E.26), de la capture.
- **Les dés de dégâts restent** : chaque arme a ses dés (E.3).
...
- **Ciblage des modules sur la grille** : les modules ont une **portée en tuiles** et une **forme d'effet** (cible unique, ligne, cône, zone en croix ou en carré) prévisualisée avant validation — l'assemblage façon Noita produit des *formes*, pas des trajectoires physiques. La ligne de vue est bloquée par le dénivelé (3.6).
...
- **Statuts de contrôle** : étourdissement et enracinement se mesurent **en ticks**, avec la durée affichée. Aucun contrôle dur ne dépasse **20 ticks** sur le joueur, et ne peut se réappliquer dans les **50 ticks** suivant sa fin (anti-stunlock), joueur comme créatures.
- **Compagnons** : ils agissent à leur propre compteur, comme toute entité. Le joueur leur donne des **consignes** (suivre / tenir la position / cibler en priorité / repli / posture agressive-défensive) modifiables à tout moment, gratuitement — donner un ordre ne coûte pas de ticks, seul l'agir en coûte.
...
### 5.2.1 Le vocabulaire des modules (et l'absence d'arbre de talents)

**Principe de progression, gravé :** il n'y a **ni arbre de talents, ni points à attribuer, ni déblocage par niveau arbitraire**. Tout s'acquiert et se spécialise par ce que le personnage *fait* et *possède* :
- les capacités viennent du **loot et de l'apprentissage** (grimoires, manuels, parchemins, enseignement de PNJ) ;
- les compétences montent **par l'usage**, régulées par le potentiel (6.4) ;
- la spécialisation vient de ce qu'on **porte** (le niveau de Mailles monte parce qu'on porte des mailles ; le niveau de Feu monte parce qu'on finit ses chaînes en Feu) ;
- le build **émerge** — il ne se planifie pas dans un écran.

**Le vocabulaire commun** (détail complet en B.4) est ce qui permet à des centaines de modules d'exister sans devenir illisibles : chacun se décrit par **six axes** — une **forme** sur la grille (ligne, cône, croix, anneau, carré, glyphe au sol...), une **portée** `[min, max]` en tuiles, une **cible**, des **coûts** (ticks, mana, endurance), des **conditions** positionnelles (plus haut que la cible, cible isolée, de dos ou de flanc, ligne de vue dégagée...) et des **effets** (dégâts, soin, statut, déplacement, terrain, invocation, glyphe).

Les **modificateurs** façon Noita opèrent sur ces axes, pas seulement sur des chiffres : étendre une forme, transformer une ligne en cône, doubler la portée en divisant la puissance, ajouter une condition contre un bonus. C'est ce qui rend l'assemblage réellement combinatoire — et ce qui permet des archétypes **positionnels** entiers (poussée, attraction, échange, portails, glyphes, invocations-murs, modelage du terrain), impossibles à concevoir proprement en temps réel et évidents sur une grille.
...
#### Assemblage — six types de modules
...
- **L'élément de la capacité dérive du vecteur de ses modules** — le Wu Xing s'applique intégralement.
- **Une capacité qui touche pose UN segment de chaîne**, quel que soit le nombre de cibles.
...
#### Familles de capacités que la grille rend possibles

Ce que le temps réel ne permettait pas de concevoir proprement, et qui devient évident ici — **tout est de la donnée, aucun coût de production supplémentaire** :
...
- **Invocations** : une créature invoquée **occupe une tuile** — elle est donc un mur, un bloqueur de vue et une menace de flanc autant qu'un allié. Raccord direct avec les compagnons (E.17) et les armes fantomatiques.
- **Glyphes et pièges** : effets persistants posés sur une tuile, déclenchés à l'entrée. Un glyphe élémentaire **pose un segment de chaîne** en se déclenchant : le Wu Xing devient positionnel.
- **Modelage du terrain** : élever ou abaisser des tuiles comme un sort — creuser une tranchée, ériger un talus, ouvrir une brèche dans un mur.

#### Sorts cataclysmiques (cratères, effondrements)
...
**Deux garde-fous de simulation :** le **plancher et le plafond de hauteur** (0 et 20) bornent naturellement le vandalisme cumulé ; la **régénération des cases sauvages** (3.3) répare le terrain hors des claims — le joueur peut défigurer le monde, le monde se soigne. Sur les cases claim en revanche, les dégâts **persistent**, ce qui rend une attaque de royaume réellement traumatisante.
...
- **Les personnages** : sprites en couches (paperdoll) — corps, tête, équipement visible par slot, teintes par instance. Le pipeline modulaire de la section 12 survit intégralement : ce sont les mêmes bibliothèques de parties, en 2D au lieu de .vox.
...
- ~~Physique voxel fine en 3D~~ **Risque levé** : destruction discrète à la tuile (voir 3.6/4.1), beaucoup moins coûteuse et bien plus simple à synchroniser en réseau.
...
- **Éclairage** (impliqué par la stat `luminosite`, A.4.5) : propagation en 2D sur la grille (bien plus simple qu'en volume), utilisée pour la visibilité nocturne, les donjons et l'ambiance. La transparence devient un simple tri de rendu.
...
**Principe :** l'assemblage des parties du corps est un **paperdoll en couches de sprites** (section 9) — chaque partie est un sprite avec ses **points d'ancrage nommés** (épaule, hanche, main, dos) et son ordre de superposition. Le pipeline modulaire est identique à celui prévu en voxel, en 2D :
...
*(Le MVP est redéfini par la direction tactique : prototype de combat isolé d'abord — document séparé — puis génération du monde en grille, ~30 matériaux, combat rapatrié, boucle de donjon. Voir l'en-tête du document.)*
...
- **Multi-ennemis** : la jauge se remplit d'**un segment par ATTAQUE, jamais par cible touchée** (sinon les groupes deviennent des générateurs de chaîne triviaux — l'XP, elle, se somme par cible). Restent à définir : les formes d'attaque qui touchent plusieurs tuiles, et si une limite d'ennemis au contact est nécessaire (l'encerclement est déjà puni par la garde frontale).
- **Vocabulaire d'attaque des créatures** : en tactique, l'intention est **affichée** plutôt que devinée — le besoin est donc moindre, mais chaque famille de créature doit avoir des actions distinctes (portée, coût en ticks, forme d'effet).
- **Fuite et désengagement** : rayon de perte d'intérêt, rupture de ligne de vue (facilitée par le dénivelé), semer en Discrétion — sans quoi chaque rencontre est un duel à mort.
...
- **Projectiles** : portée en tuiles, ligne de vue, formes d'effet, friendly fire en coop.
- **Endgame « Dark Continent »** (idée retenue, non spécifiée) : au-delà d'une barrière géographique, le même monde continu avec des **règles** différentes — corruption au-delà de 100, météo létale, nourriture qui se corrompt, endurance qui ne régénère pas normalement, statuts insoignables sur place. Une **expédition** que le royaume finance, pas une zone à nettoyer. Coûte du contenu, presque aucun système.
...
- **Aucun arbre de talents, aucun point à dépenser** : les modules s'obtiennent par le **loot et l'apprentissage** (5.1), montent de niveau **par l'usage** (6.4), et le build **émerge** de ce qu'on possède et de ce qu'on utilise.
