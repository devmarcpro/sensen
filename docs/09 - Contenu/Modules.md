...
> [!success] Décidé et codé le 2026-08-30 — la matrice 5 × 3 se différencie, et deux familles de plus
> Le designer a tranché « que tout se différencie vraiment, je te laisse faire ». **La matrice** : les paliers ne se distinguaient que par le dé. Désormais le palier **léger** (1d4, 3 ticks) reste le sort de harcèlement — bon marché, rapide, sans effet ; le palier **moyen** (2d6, 8 ticks) gagne **+1 dé de bonus** et devient le sort de référence ; le palier **lourd** (4d6, 16 ticks) porte la **signature de son élément** en statut : *Brasier* brûle, *Banquise* ralentit, *Éboulement* étourdit, *Fonte* fait saigner, *Foudroiement* enracine. Cinq lourds, cinq lectures du même prix. La matrice reste (chaque case est une munition à collectionner — les charges lui donnent son sens), mais on ne choisit plus le palier pour le chiffre seul.
> **Les familles manquantes.** Déclencheurs et liaisons n'en avaient pas : les déclencheurs se rangent en **`evenement`** (À l'impact, Curée, Dérobade, Ouverture, Parade, Riposte, Testament, Veille, Accord) et **`minuterie`** (Cadence, Mèche, Sceau) ; les liaisons en **`cibles`** (Dispersion, Meute, Miroir, Partage, Propagation, Ricochet, Salve, Contagion) et **`rejeu`** (Boucle, Écho, Répétition, Alternance). Le champ `famille` de chaque fiche le porte, le dossier le reflète, l'écran *Composer* l'affiche.

> [!success] Codé le 2026-08-30 — les deux limites de la veille sont levées : le revers de l'Écaille, le Feu de la Trempe
> **Écaille élémentaire** portait son immunité mais pas sa contrepartie (« vulnérabilité +50 % à l'élément qu'il domine »). Le statut porte désormais un modificateur **`vulnerabilite` (×1,5)** ; `_appliquer_degats` l'applique quand l'élément dominant du coup est **celui que l'élément de l'écaille domine** (table `wuxing.domine` : une écaille de Feu laisse passer le Métal amplifié). **Trempe** portait son +1 dé mais pas son élément : un modificateur **`element_arme` (grant: feu)** remplace le vecteur de l'arme tant que le statut tient — dans `_vecteur_arme_de`, le même point d'entrée que la Communion des cinq du Souffle, et l'assembleur de capacités lit désormais ce vecteur-là (les noyaux d'arme héritent donc de la Trempe). **Décision** : les deux passent par des **modificateurs de statut** lus par le résolveur, pas par un champ codé en dur — un objet ou un talent pourra accorder une vulnérabilité ou changer l'élément d'une arme sans une ligne de plus.
