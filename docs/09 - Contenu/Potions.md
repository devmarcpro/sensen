...
> [!success] Codé le 2026-08-28 — treize potions
> Ajoutées : **résistance au feu** (menthe : statut `isolation` +40 contre la chaleur), **résistance au froid** (ortie : +40 contre le froid — les deux lus par le confort thermique, `Etres.add_statuts(e, "isolation")`), **vision nocturne** (belladone : un statut peut désormais **accorder un tag** — modificateur `{"cible": "tag", "grant": …}` lu par `Etres.recalculer`), **antipoison** (camomille : `purge:poison` + tag `immunite_poison` accordé 5 min), **poison de lame** (amanite : statut `lame_empoisonnee`, modificateur `attaque_statut` → chaque coup d'arme applique Poison ; **illégal** : `absurd_laws_pool.substances_illegales` — 80 % des royaumes qui punissent le meurtre ont une loi `objet/poison_de_lame` (confiscation), et **l'appliquer sur sa lame est une infraction** comme l'acheter ou le ramasser). **Respiration aquatique** (roseau) est venue avec la nage (Eau et liquides) : les 12 + charisme, soit 14 potions.
