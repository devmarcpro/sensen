...
> [!success] Mesuré et corrigé le 2026-09-07, 18 h 15 — la génération d'un objet : `objet.generer` 12,7 → 7,7 ms, le butin des coffres 47 → 31 ms, l'étage à chaud 64 → 43 ms
> Après le plein en bloc, le gros morceau d'un étage est le butin de ses coffres (40 à 47 ms), et dans le butin `objet.generer` (12 ms pour 82 pièces). La lecture de `Generateur.generer` trouve trois fois le même gaspillage que le 7 h — **refaire à chaque appel ce qui ne change jamais** : `_affixes_pour(slot)` balaie tout le catalogue des affixes **pour chaque objet** ; `rarete_pour` recalcule le plafond de la table des raretés en balayant ses clés **pour chaque objet** ; `_composer_parchemin` reconstruit et trie les modules par type **pour chaque parchemin**. Les trois sont mis en cache à la première demande, dans le même ordre de parcours — donc le même tirage rend le même objet : `sonde_butin` avant et après, ligne pour ligne. Ce qui reste dans le butin : `comp.tirage` (11 ms, la pondération et son parcours linéaire de deux cent quarante poids par tirage — un cumul et une recherche binaire rendraient le même matériau sauf à la frontière d'un arrondi, ce qu'il faudrait prouver avant), `comp.appliquer` (4 ms), et les 7,7 ms restants de `generer` (les affixes tirés au filtre, la copie profonde de la base).
