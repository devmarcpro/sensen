...
> · `tools/gen_materials.py` — **tables → fiches**. C'est le chemin normal. Toute retouche d'une matière se fait dans la table.
> · `tools/regen_catalogues.py` — **fiches → tables**. Le chemin inverse. Il **refuse maintenant de s'exécuter** sans `--vraiment` : c'est un outil de sauvetage pour rattraper une donnée qui aurait dérivé, une fois, en connaissance de cause. Le laisser tourner sans le dire, c'est retourner la flèche en silence et perdre la décision.
> **Les deux garde-fous, à lancer après toute retouche d'un catalogue, de la palette ou d'un générateur** : `tools/verif_generateurs.py` (il relance la chaîne et compare **chaque champ de chaque fiche** — il sort en échec à la moindre différence) et `tools/regen_catalogues.py --verifier` (qui n'écrit rien et dit si les tables et les fiches se sont éloignées).
> **L'état au moment de la décision, et c'est ce qui la rend gratuite** : les deux sens s'accordent **exactement**. Douze catalogues « à jour », 0 ajout, 0 correction, 247 matières des deux côtés. La question annonçait « il faut alors valider à la main les 94 lignes reversées et les chiffres étirés » — ce travail a été fait le 2026-09-08, la décision ne fait que le sceller.
> **Ce que la décision a fait tomber en s'appliquant** : `regen_catalogues.py` portait encore un en-tête à **treize** colonnes et aurait effacé les 247 points de fusion écrits le matin même — un outil du chemin inverse qui ignore une colonne ne la laisse pas tranquille, il la supprime. Et son `--verifier` annonçait **six** matériaux « à ajouter » à chaque passage, faute de connaître les cinq alias d'identifiant et le nom à rallonge du guano. *Un vérificateur qui crie toujours au loup n'est plus lu — c'est la même maladie que la suite qui disait « tout passe ».*
> **Ce que la décision ne couvre pas** : huit champs des fiches n'ont **aucune note pour source** — `palier`, `palier_fixe`, `stats_base`, `sous_categorie`, `tags`, `noise`, `wuxing`, `harvest`. Le générateur les *préserve* au lieu de les écrire, et `sous_categorie` est lu par le jeu sur 76 fiches. Les reverser dans des notes est le prolongement de la décision, et ce n'est pas fait.

> [!bug] 2026-09-09 — trois Godot écrivaient dans le même fichier de résultats, et j'ai douté d'un échec vrai
> La règle du projet est **une instance de Godot à la fois** ; je l'ai enfreinte en lançant une suite de fond puis deux vérifications ciblées pendant qu'elle tournait. Or `lancer_tests.py` écrivait sa sortie dans `build/tests.txt` — **le même chemin pour tout le monde**. Les trois exécutions s'y sont mélangées, et j'ai passé un moment à me demander si l'échec que j'y lisais appartenait au code que je venais d'écrire ou à un autre run.
> **Il était vrai** — mais je ne pouvais pas le savoir, et c'est exactement le coût : *un outil de vérification qui mélange deux résultats est pire qu'absent, parce qu'il fait chercher un défaut qui n'existe pas.*
> Deux corrections : la sortie porte le **PID** (le dernier résultat reste recopié dans `build/tests.txt`, pour rester facile à trouver), et l'outil **refuse de démarrer si un Godot tourne déjà** — `--quand-meme` passe outre. La règle n'est plus espérée, elle est tenue.

> [!bug] 2026-09-09 — une erreur de script tuait un test au milieu, et la suite disait « tout passe » — `tools/lancer_tests.py`
> **Trouvé en passant**, dans la sortie d'une suite verte : `test_gaz_dans_le_sol` lisait `cfg.gaz` sur `gaz_regles`, qui n'a pas cette clé — les fiches des gaz ont été sorties dans `data/gaz/` le 2026-09-07, et **six lignes du test** sont restées sur l'ancien chemin. Godot répond *Invalid access to property or key* et **interrompt la fonction** dès la première : tout ce que le test devait vérifier ensuite — le nuage qui sort de la poche percée, le gaz toxique qui blesse, le grisou qui explose au contact de la lave, et les quinze fiches passées en revue — n'a **plus été joué depuis deux jours**. Rien n'a rougi : une erreur de script n'échoue nulle part. La suite comptait ses `verifier` et concluait « TESTS : tout passe ».
> **Ce n'est pas le test qui manquait de rigueur, c'est le compteur.** Un test qui meurt en route est indistinguable, pour la suite, d'un test court.
> **L'inversion qui fait mal** : les **sondes** avaient déjà ce garde-fou depuis leur balayage (`balayer_sondes.py` compte les `SCRIPT ERROR` de chacune) ; la **suite**, la vérification la plus regardée du dépôt, ne l'avait pas. On surveillait les outils d'observation mieux que la preuve.
> **`tools/lancer_tests.py`** joue la suite, refuse toute *SCRIPT ERROR*, *Parse Error* ou *Compilation failed* — en disant dans quelle fonction et quel fichier —, exige la ligne de bilan, et sort en code non nul dès qu'il manque quelque chose. C'est lui qui lance la suite désormais ; `--seul fragment` passe au travers.
> **Et le test corrigé vérifie maintenant TOUTES les poches** : un `break` n'en regardait qu'une.
> **Une leçon de plus, payée sur-le-champ** : ma propre correction a d'abord introduit une erreur d'analyse, et l'outil tout neuf a attendu son délai **entier** — une heure — parce qu'un script qui ne compile pas laisse Godot tourner sans jamais rien jouer. Il surveille désormais la sortie pendant qu'elle s'écrit et coupe dès que le moteur dit qu'il n'a pas pu charger la suite. *Un garde-fou qui met une heure à dire non n'est pas un garde-fou.*


> [!success] Codé le 2026-09-07, 21 h — `tools/verif_callouts_codes.py` : un callout « Codé » qui ment (designer : « vérifie aussi les notes marquées Codés, je pense qu'il y a de faux positifs ou des systèmes codés à moitié »)
> Le coffre porte **720 callouts `[!success]`**. `verif_doc_code.py` balaie la note entière ; personne ne vérifiait ce qu'un callout **affirme avoir codé** — et c'est bien plus fort : une citation morte y est une promesse non tenue, pas une note qui a vieilli. L'outil relève les identifiants cités en `code` dans ces callouts (fonction, méthode C++, variable membre, clé de données, fichier, **test**) et dit lesquels le dépôt n'a pas.
> **Ce qu'il a trouvé sur 362 citations vérifiables : trois citations vieillies, aucune promesse non tenue.** **Surface._poser_village** (la pose d'agglomération a été refondue avec les quartiers : c'est `generer_cellule` et `_poser_batiment`), **Monde.materialiser** (qui n'a jamais existé sous ce nom : les rails et les quais sont posés par `_poser_cellule`), et `systems/worldgen/camp.gd` (il n'y a pas de fichier de ce nom : le camp se charge depuis `sim_lieux.gd`). Les trois notes sont corrigées, la correction datée dans la note elle-même. **Le reste tient** : les systèmes annoncés existent.
> **Deux règles apprises à l'outil, qui disent quelque chose du coffre** : un callout dont le **titre est barré** (`~~…~~`) annonce ce qui a été *retiré* depuis — ses citations n'engagent plus (c'est le cas de la timeline, supprimée par le designer le 2026-08-30) ; et un **test_x** cité doit être **écrit ET inscrit au lanceur**, ou exister comme scène à part — c'est la marque exacte d'un « codé à moitié » : une note qui dit « vérifié » alors que rien ne le vérifie. Aucun n'a été trouvé.
> **La limite, qu'il faut dire** : l'outil vérifie qu'un identifiant *existe*, pas qu'il *fait ce que la note promet*. Un système à moitié codé dont les noms sont justes lui échappe — ce sont les « ce que ça ne fait pas encore » des notes qui le disent, et il y en a plus de cent.
