...
> - **Cinquante-et-une plantes**, chacune avec **ses** nombres (`tools/gen_agriculture.py` les écrit depuis une table lisible ; les fiches restent des données qu'on retouche à la main) : `duree_jours`, `recolte_base`, `nutrition`, `saisons` de semis, `besoin_eau`, et deux champs nouveaux — la **`famille`** (céréale, racine, légume, légumineuse, fruit, fibre, oléagineux) et les **`conditions`** : les `biomes` (des tags de biome) où elle pousse à plein, et la `fertilite_min` sous laquelle un fermier ne la sème pas. Sept céréales (du seigle du froid au riz des marais et au maïs des plaines chaudes), quatre racines, sept légumes, quatre légumineuses, deux fibres, deux oléagineux, la canne à sucre, et quinze fruits — des buissons et des arbres (pomme, olive, datte, noix — la plante porte le nom de ce qu'on récolte, jamais celui d'un bois du catalogue) qui vivent comme le verger, plantés une fois.
> - **Ce que les conditions font** : `plante_a_semer` ne retient, parmi les cultures de saison, que celles dont les biomes admettent la cellule et dont la terre a la fertilité — un fermier sème ce qui vient chez lui, et s'il n'a rien qui convienne, il sème ce qu'il a ; une plante hors de son climat rend `champs.hors_climat.rendement` (la moitié). La **rotation** compte la **famille** et non l'espèce : semer du seigle après du blé n'est pas tourner. Et une **légumineuse** ne fatigue pas la terre, elle la nourrit : à sa récolte, la fertilité **monte** de `champs.legumineuse.fertilite_rendue` au lieu de baisser — c'est la rotation triennale des vrais champs.
> - **Les tables par biome sont dérivées** : `champs.cultures_par_biome`, les buissons des vergers et `enclos.especes_par_biome` sont écrits par le générateur depuis les conditions des plantes et les biomes du bétail. Une table de moins à tenir à la main, et une ville de désert n'a plus de chou.
> - **Quatorze bêtes domestiques** (`data/creatures/bete/`, tag `domestique`) — vache, mouton, chèvre, cochon, poule, oie, canard, lapin, âne, cheval, buffle, yak, lama, dromadaire — chacune avec son bloc **`elevage`** : ses `produits` (le lait de la vache, la laine du mouton au printemps, les œufs de la poule, le crin du cheval l'été), son `fourrage` (un buffle mange trois fois ce qu'une poule mange), sa `naissance_chance` (le cochon et le lapin pullulent, le cheval et le dromadaire non), son `abattage` (ce qu'il rend à la boucherie) et ses `biomes`. `_semaine_betail` lit ces nombres sur la fiche, et retombe sur `villes.enclos` pour une espèce sauvage. L'œuf est un aliment nouveau.
> **Ce que ça ne fait pas encore**, et qui suit le même fil : les transformations (le lait en fromage, l'olive et le tournesol en huile, la canne en sucre, le lin en toile, le grain en pain — des recettes de station, pas de l'agriculture), les maladies des bêtes et des plantes, le pâturage tuile à tuile. Et **à juger** : tous les nombres de la table, écrits pour être plausibles, pas mesurés.

> [!important] Décidé le 2026-09-07, 20 h 10 — le jardin du joueur : labourer, arroser, planter un arbre (designer : « ok fais le nécessaire »)
> Je lui ai décrit l'agriculture telle qu'elle marchait et j'ai dit ce qui manquait au **joueur** : il ne pouvait que semer sur une cellule au rôle *champs*, sans jamais préparer la terre ni l'arroser, et le verger ne naissait qu'avec les villes. Trois gestes comblent cela, tous en données (`combat_rules.royaume.agriculture`) :
> - **Labourer** (`labourer`, option sur une tuile de terre voisine, `labour.ticks` avec l'outil de récolte en main) : la tuile devient une **parcelle nue** prête à semer, et la fertilité de la terre monte de `labour.fertilite`. C'est ce qui remplace la contrainte du rôle *champs* : **on peut désormais planter sur toute tuile labourée**, où qu'elle soit — sur une cellule *champs*, on plante encore directement, comme avant.
