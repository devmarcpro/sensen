...
- **Collecte de 97 objets en six étages** (robot invincible, 2026-09-02 — `--invincible --inventaire`) : 59 combats, 179 kills, 47 ramassages. Ce que le butin montre, chiffres à l'appui, et qui relève de ton arbitrage :
  1. **41 % du butin est une « Fiole {apparence} »** — 40 des 97 objets, toutes identiques et non identifiées. Le loot est noyé sous les potions inconnues ; il faudrait soit baisser leur poids de tirage, soit permettre d'en identifier plusieurs d'un coup.
  2. **Les boucliers ne sont pas assemblés** : les six ramassés sont tous « Bouclier », qualité 1,00, bois 100 %, **sans composants ni matériau** — alors que les armes et les armures le sont (« Épée en Fer (chef-d'œuvre 2.13) », trois composants, deux affixes). Même chose pour les **bijoux** et les **gemmes**, tous à qualité 0,00.
  3. **Toutes les armures pèsent 6,0 kg**, casque comme cuirasse comme jambières comme brassards : le poids d'une armure est une constante, pas une somme de ses composants.
  4. **179 ennemis tués, zéro pièce d'or.** Les hostiles ne lâchent rien de monnayable.
  5. **43 sorts refusés sur 52 lancés** par le robot — à creuser : ses sorts composés au hasard tombent souvent hors de portée ou sans ligne de vue.
  Le détail complet est dans `rapports/butin_2026-09-02.txt` (une ligne par objet : type, nom, qualité, rareté, poids, Wu Xing, affixes, composants).
