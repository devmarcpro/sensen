...
**8. Sur cinquante-deux semaines, la base se vide entièrement** (même sonde, `--semaines 52`) : 16 résidents à la douzième semaine, 12 à la seizième, 9 à la vingtième, 5 à la vingt-quatrième, 3 à la vingt-huitième, **0 à la trente-deuxième**. La dette monte à 2 738 or et y reste, rien ne la paie. Neuf migrants sont arrivés dans l'année — chacun reparti la semaine suivante, chassé par le même palier de dette. Le stock de bois (77 à la douzième semaine) fond à zéro par les raids, qui en prennent la moitié à chaque passage. Ce n'est pas une surprise après le point 1 : c'est sa conséquence jusqu'au bout, et la raison de trancher l'entretien avant tout le reste. *(Deux écarts de code trouvés au passage et corrigés : l'humeur s'empilait sous zéro en dette, et les partants restaient plantés dans le résidentiel — voir [[Entretien et taxes]].)*
...
**10. Un étal garni du bois de la base rapporte quarante or par semaine** (`sonde_grande_base --etal`, monde 9) : trente villageois à trois cellules font 3,25 clients par heure (546 par semaine), mais le bois de la semaine part en **une seule pile** — un client l'emporte, une vente par semaine, 465 or en douze semaines. Contre 190 d'entretien. Deux questions : est-ce voulu qu'une pile de trente chênes bruts parte d'un coup au prix de la pile (« chaque client tire un objet de l'étal ») ? Et si l'étal vendait à l'unité, la base se paierait presque (30 chênes à ~3 or = 90 or par semaine par zone de bois) — c'est l'autre réponse au point 1.

**11. L'écran Assigner propose « Maître de guilde » et « Prêtre »** à n'importe quel résident de la base, comme les autres fonctions du catalogue. Un maître de guilde nommé ainsi offre des quêtes ; est-ce voulu sans hall de guilde ni chapelle, ou ces deux fonctions devraient-elles rester aux villages ?
...
## 2026-09-05 — « Certaines armes n'ont pas de craft » (designer, 1 h)

Vérifié dans les données : **les trente-six armes assemblées ont chacune une recette** (station + compétence), et chacun des dix-sept composants a au moins une recette débloquée d'emblée (le métal, le bois, la roche taillée, le cuir, la fibre ; vingt-six variantes — obsidienne, os, verre, ivoire, soie, météorite… — se débloquent par le butin de donjon, un marchand spécialisé, la guilde des forgerons ou un PNJ). Ce que le joueur voit, c'est autre chose : **l'atelier ne liste que les recettes des stations à portée** (portées dans le sac ou adjacentes — [[Stations de transformation]]), et le kit de départ n'a que l'établi. À l'établi manquent donc **treize armes** : dix demandent la **forge** (sabre, rapière, stylet, hache d'armes, marteau de guerre, fléau, hallebarde, pistolet, cymbales, pavois), trois l'**atelier de tissage** (luth, vielle, tambour) — et quatre objets de main gauche la **table d'enchantement** (orbe, sceptre, talisman, grimoire de main). Ces stations se fabriquent à l'établi (pierre taillée, lingots, planches) et se portent ou se posent.

Deux choses à trancher :
1. **Montrer ou cacher.** Aujourd'hui une recette dont la station manque n'apparaît pas du tout : rien ne dit au joueur qu'un sabre existe ni qu'il lui faut une forge. L'alternative est de lister ces recettes **grisées**, avec la station requise — un changement de l'écran d'atelier (que le designer a fait refondre le 2026-09-04), pas des données. Je ne l'ai pas fait sans son mot.
2. **Les vingt objets « de fortune »** (`proto_épée`, `proto_arc`, `proto_masse`, casques et cuirasses de cuir, outils en fer…) : ce sont les pièces du prototype, une matière fixe, sans composants ni recette. Hors des tests et d'une vieille recette de seau (`recipes/fabrication/fabriquer_seau.json`, encore listée à l'établi : elle rend un `proto_seau` sans composants alors que `craft_seau` s'assemble d'une plaque et de sangles — à retirer avec les autres, ou à garder comme seau de fortune), rien ne les fabrique ni ne les fait tomber. Pour les sprites, ce sont les mêmes silhouettes que leurs bases assemblées ; pour le jeu, à retirer ou à garder comme butin de fortune — c'est une décision de contenu.

## 2026-09-05 — Quatre demandes du designer de 13 h : les chiffres que j'ai choisis, et comment les défaire

1. **Le retour de coup** (tremblement + rouge) : `styles.coups` — 2,5 px pendant 0,22 s, rouge `#ff6a5a` qui s'éteint en 0,16 s. Plus fort, plus long, plus rouge : quatre valeurs.
...
- **Douze mois nommés par les animaux ; sept jours nommés Soleil, Lune et les cinq éléments.** *Tranché à 16 h 40 : « 12 mois de 30 jours »* — l'année fait 360 jours, les saisons ont suivi. Les noms restent à toi (`mois`, `jours_semaine`).
...
- **Ce qu'une ville paie** : mesuré sur la graine 9, une ville de 126 résidents devrait 1 360 or par semaine de gages et de structures pour 104 or de revenu — elle était en faillite la première semaine. J'ai donc **exempté de l'entretien les villes qui ne sont pas au joueur** (leur budget propre viendra avec l'économie, B3 : taxes et dépenses). **Quand le joueur prend une ville, il paie ses gages** (« un camp et une ville sont identiques ») : 1 260 or par semaine pour cette ville-là — c'est énorme, et c'est à toi de dire si prendre une ville doit coûter cela, ou si les résidents d'une ville prise vivent de la ville (taxes) et non du joueur. Le levier immédiat : `entretien_pnj` ; le levier de fond : le budget de ville de B3.

## 2026-09-05 — Les villes vues (B1, 17 h) : ce que j'ai regardé et ce qui est à ton œil

- **La grille de rues** (deux axes plus deux parallèles dès le bourg, neuf îlots) se lit bien en capture : la place au centre, les îlots bâtis, les arbres entre. Les toits sont ceux de la palette du biome (bois sombre en forêt tempérée) : tous les quartiers se ressemblent de loin. Une palette par quartier (les ateliers en pierre, les chaumières en chaume) serait deux entrées de données par quartier — si tu veux.
...
- *(Périmé le 2026-09-07 — le bétail sauvage « domestiqué » ci-dessous a été remplacé par quatorze bêtes domestiques : vache, mouton, chèvre, cochon, poule… ; les espèces par biome sont dans `villes.enclos.especes_par_biome`. Relevé le 2026-09-13, le coffre qui se contredit.)* **Un enclos par quartier agricole**, deux à quatre bêtes, la laine et le lait comme produits (une unité de laine par mouflon et par semaine, deux de lait par bison, une par renne ou chameau). Les espèces par biome sont une table aussi ; le sanglier, le canard et le cheval ne donnent rien tant qu'il n'y a ni œuf ni viande hebdomadaire — dis si tu veux que le bétail se mange (une bête abattue par semaine → viande).
...
## 2026-09-05 — Le lag en ville et le C++ (21 h)

- **Ta règle dit « pas de GDExtension »** (`AGENT.md`). Réécrire en C++ est donc ta décision, pas la mienne. Avant de la prendre : la simulation ne coûte presque rien à l'image en ville (0,01 ms par image), c'est le **dessin** qui coûte — j'ai ramené une cité de 247 habitants de 25 à 16,6 ms par image (pire : de 83 à 18 ms) sans C++, en ne redessinant que ce qui change. Le plancher restant est le rendu de deux cents paperdolls et du terrain en commandes de canvas.
- **Ce que je ferais ensuite, en GDScript** : le terrain par cellule (un nœud par cellule, redessiné seulement quand elle change : plus de saccade quand on marche), les êtres lointains en pictogramme (une commande au lieu de trente), et le texte du bloc d'information seulement quand il change. Si après ça la ville lag encore chez toi, dis-moi ta machine et le nombre d'images par seconde que tu vois, et on parlera de C++ (une GDExtension pour la grille et les chemins serait le bon endroit).
- **Le jour de départ** : le 3 du Rat (pas de fête). Le Nouvel An au premier matin était à moi, pas à toi.

## 2026-09-05 — Fragmentation et C++ (21 h 15) : ce que j'ai décidé pour toi

Tu m'as laissé trancher ; voici les trois choix et où les changer ([[Modules de la simulation et le C++]]) :
...
- **La place meublée** : fontaine, torchères (elles éclairent la nuit), statue d'une capitale, étals au quartier marchand. À juger : la densité du mobilier.
- **L'agriculture.** Les cultures ont des saisons de semis (rien ne se sème l'hiver), la rotation change la culture d'une récolte à l'autre, la jachère repose le champ après quatre récoltes, l'irrigation (une eau à trois tuiles) donne du rendement et protège de la canicule — et la génération pose désormais les champs **aux abords et au bord de l'eau**. **Ce que je te laisse** : les nombres (`villes.json → champs`), et surtout la question de fond — veux-tu que le joueur voie ces cycles (un champ nu l'hiver, une jachère qui se voit) ou qu'ils restent une comptabilité de fond ?
- **Le troupeau.** Il mange le fourrage des stocks, produit à sa saison (la laine au printemps), fait des petits quand il est nourri et au large, perd une bête quand il a faim, et son surplus part à la boucherie. À juger : le rythme (une naissance par bête et par semaine à 12 %, capacité 8 par enclos) et la dureté de la famine (une bête morte par semaine sous la moitié du fourrage).
- **Les bras d'une ville.** Un lieu de travail emploie plusieurs personnes (deux à l'atelier, quatre à la caserne…), une zone de récolte emploie à la mesure de sa richesse. Le nombre d'oisifs baisse mais **reste élevé** (une centaine sur deux cents dans une ville de 188 habitants) : ce sont les enfants, les vieux et ceux que la ville ne peut pas employer. **À trancher** : est-ce acceptable, ou faut-il des métiers sans bâtiment (portefaix, journaliers) et des saisonniers aux moissons ?
- **Une ville minière a désormais du minerai.** Mesuré : Ouarbah, cité minière de 247 habitants, n'avait **pas un filon** dans ses sept cellules — donc aucun mineur, aucune zone de récolte, et ses bras en trop restaient oisifs. La vocation se lit sur la couche `ressources`, les filons se posaient sur un seuil plus haut : les deux règles ne se parlaient pas. J'ai fait suivre le sol à la vocation (seuil abaissé, densité triplée, seulement dans les cellules d'une ville minière). **À juger** : le dosage (`vocations.liste.miniere.filons`) — une cité minière doit-elle avoir des veines partout, ou juste assez pour occuper ses mineurs ? Et la contrepartie manque encore pour la vocation **forestière** (les arbres viennent des listes du biome, pas d'un seuil) et la **portuaire** (un port sans poisson).
- **La vocation d'une ville.** Chaque agglomération vit de ce qui l'entoure : minière, forestière, portuaire, grenier, pastorale, marchande — ou commune, et beaucoup le sont (42 % du monde 9). Elle change les boutiques servies en premier, le nombre d'ateliers et de champs, les zones de récolte et un ou deux bâtiments du centre (un comptoir au port, une écurie au carrefour). **À juger** : les poids (`villes.json → vocations`) et la part de villes sans vocation ; faut-il que la vocation se dise à l'écran (un mot sous le nom de la ville) ou qu'elle se lise seulement dans ce qui est bâti ? Et faut-il qu'elle bouge (une mine s'épuise, un port s'ensable) ?
- **Une ville pleine cesse de grossir.** Une année simulée faisait passer une ville de 198 à 247 habitants pour 189 lits, sans que personne s'en plaigne. À la majorité, un enfant quitte désormais le lit de ses parents quand la ville a dépassé ce que son bâti loge : il perd le confort d'un logement, son humeur baisse, et la migration (qui existait) fait le reste. **À juger** : préfères-tu qu'une ville riche **bâtisse** au lieu d'exporter ses enfants ? Cela demande de décider qui paie et où l'on pose la maison — je ne l'ai pas tranché. Et l'accumulation des stocks est un autre sujet : une ville finit l'année avec 12 744 baies et 6 000 pièces au trésor, sans rien qui les use ni les dépense.
> [!warning] Vu sur les captures du 2026-09-07, 13 h (`capture.tscn -- --palier village --graine 9 --sur cimetiere|verger`)
> - **Le verger se lit** : des rangs de buissons vert sombre, nettement distincts du damier clair d'un champ et de la terre nue. Rien à faire de plus.
> - **Le moulin ne se lisait pas non plus** — c'était une maison de plus. Corrigé sans art : il fait désormais trois niveaux (deux greniers au-dessus), il domine les toits et se voit de loin. Si tu valides le principe, un beffroi, un phare ou un donjon suivront la même règle.
> - **Le cimetière ne se lit pas.** Ce qu'on voit est une cour clôturée avec des cubes gris dedans — la même clôture qu'un enclos à bêtes, et des tombes qui ressemblent à des pierres. Le moteur dessine un meuble comme un bloc coloré dont la hauteur suit l'emprise : une stèle (petite au sol, haute) n'est pas exprimable ainsi. **À trancher** : veux-tu un sprite pour la tombe (et une clôture différente pour le cimetière), ou le cimetière peut-il rester une chose qu'on découvre en passant le curseur ?

- **Les tombes portent un nom.** Quand un habitant d'une ville meurt, il est enterré dans le cimetière de SA ville, où qu'il soit tombé ; la tombe dit « Ci-gît Prénom NOM, métier, an X » au survol. Le cimetière a une capacité (trois tombes au hameau, vingt à la cité) et n'accepte plus personne une fois plein. **À juger** : est-ce que ça se remarque en jeu, ou faut-il que quelque chose l'annonce (une ligne au journal, un PNJ en deuil) ? Et faut-il que tuer un habitant et voir sa tombe pèse sur la réputation ?
- **Le verger.** Un enclos de buissons (framboisier, myrtillier, vigne, houblon selon le biome) aux abords des quartiers agricoles et résidentiels, et au bord d'un hameau. On le plante une fois : ni rotation, ni jachère, et ses tuiles ont leur vert sombre qui rougit à maturité. **À juger** : faut-il de vrais arbres fruitiers (ils bloqueraient la vue), et faut-il que la vigne donne du vin (il y faudrait une cuve et une recette) ?
- **Les repères.** Un puits par quartier (contre une rue quand le quartier n'a pas de place), un cimetière clos de tombes avec sa grille — contre la chapelle, hors les murs quand la ville est fortifiée —, un moulin aux quartiers agricoles et aux villes-greniers. **À juger** : le nombre de tombes par palier (trois au hameau, vingt à la cité), et s'il faut d'autres repères (lavoir, halle couverte, gibet, fontaine).
- **Les îlots.** Les rues serpentaient si bien qu'elles ne laissaient plus de terrain constructible : l'écart entre deux rues est maintenant tenu à treize tuiles. **À regarder** : est-ce que la ville respire encore, ou est-ce qu'elle a repris un air de quadrillage ?
- **Ce que j'ai décidé sans toi, et que tu peux défaire** : le centre loge moins qu'avant (`parts.centre` 1,0 → 0,75, résidentiel 1,4 → 1,6) parce qu'il est plein d'institutions ; les boutiques d'une ville se répartissent entre ses quartiers (le centre en garde la moitié) au lieu de s'entasser sur la place.
...
- **Ce que je n'ai pas choisi pour toi** : un toit à pans (deux versants, un faîte) plutôt qu'un plat — c'est un dessin de plus, dis si tu le veux ; la matière du toit par biome (la palette de tous les biomes dit « chaume tressé » aujourd'hui : de la tuile en ville, de l'ardoise en montagne, ça se change dans `village_palette` des biomes) ; la hauteur d'un niveau (deux blocs, `NIVEAU_BLOCS` dans `main.gd`) ; si les toits doivent devenir translucides quand un compagnon est dessous.
- **Ce que ça ne fait pas encore** : les gens qui logent en haut (point 99), les intérieurs persistants. Et la ligne de vue de la simulation n'a pas changé : un mur coupait déjà la vue, il la coupe pareil.

## 2026-09-06 — « Réécriture en C++ et optimisation » (10 h) : ce que j'ai fait de ta phrase

- **Ce que j'ai lu.** Pas « réécris le jeu en C++ » : quatorze mille lignes de règles portées figeraient le design et casseraient la suite, et la mesure d'hier disait que le C++ ne gagnerait rien sur l'image en ville (c'est le rendu). J'ai fait ce que la note [[Modules de la simulation et le C++]] avait décidé pour le jour où tu le demanderais : **le noyau de la grille en C++**, et de l'optimisation là où le coût était algorithmique. Si tu voulais bien une réécriture complète, dis-le noir sur blanc et on en parle chiffres en main — mais je te déconseille.
- **Ce qui existe.** Une GDExtension, `sensen_grille` (`cpp/`, bâtie par `tools/build_cpp.ps1`, la DLL versionnée) : chemins, atteignables, lignes et champ de vue, composantes — **deux à trois cents fois plus vite** que le GDScript, et **exactement les mêmes résultats** (`test_noyau_cpp` compare paire par paire ; un chemin différent à coût égal changerait où un PNJ se tient). Sans la DLL, le jeu tourne tout en GDScript. Et l'ordonnanceur du monde ne balaie plus toutes les entités à chaque pas (une file triée validée en tête).
...
## 2026-09-07 — Le sol répond à la pioche, la ferme devient réelle : tous les nombres sont à toi

Le soir a ajouté trois systèmes de contenu réel ([[Gaz dans le sol]], [[Agriculture et élevage]], [[Cuisine et alchimie]]). Les **règles** sont décidées et testées ; les **nombres** ont été écrits pour être plausibles, pas mesurés en jeu. Ce que je te demande de regarder, dans l'ordre où ça se sentira :

1. **Le grisou.** Un mineur du kit de départ porte une lumière (2) : il allume le méthane dès le premier pas d'automate dans le nuage, et l'explosion fait 4d6 sur un rayon de 2. Un mineur qui meurt à son premier coup de pioche n'apprend rien — faut-il que la première fois avertisse (un sifflement, une ligne de journal quand la poche est *voisine*) ? Et la densité : 137 poches de gaz et 47 autres à l'étage 8 d'une mine (seuil 0,72 / 0,76 / 0,80) — est-ce une mine ou un champ de mines ?
2. **La ferme.** Chaque plante a ses nombres (`tools/gen_agriculture.py`, une table lisible) : durée en jours, récolte par parcelle, nutrition, saisons, eau, fertilité minimale. J'ai réglé pour que les céréales nourrissent et que les épices soient rares ; rien de tout cela n'a été joué. Une ville de désert vit-elle du millet et du sorgho ? Une ville de montagne du seigle et du quinoa ?
3. **Le bétail.** Le fourrage par bête (1 pour une poule, 3 pour un buffle), les naissances (0,05 pour le cheval, 0,35 pour le lapin), les produits (3 laits par vache et par semaine, 3 œufs par poule). Une ville de plaine avec dix vaches nourrit-elle trop de monde ?
