...
## 2026-09-05 — La directive de simulation (98) : quatre arbitrages qui changent ce qu'est le jeu

Le designer a promu la simulation au rang de critère de jugement, avec onze références au lieu de sept ([[Simulation de vie et de monde]]). L'audit des neuf axes est écrit et mesuré ; six axes sont ajoutés au programme. **Quatre points ne sont pas des réglages** — ils décident de ce qu'est Sensen — et je ne les tranche pas.

1. **La blessure qui dure (axe I) contre « pas de permadeath ».** [[Mort et pénalité]] est une décision fondatrice : on pénalise l'économie, jamais la progression. Kenshi tient à ce qu'on perde un bras et qu'on continue à jouer, et c'est de là que vient son goût. Les deux peuvent cohabiter — un estropié n'est pas un mort — mais il faut le dire, parce que ça change ce qu'on ressent en perdant un combat. **Trois options** : (a) rien ne survit au combat, comme aujourd'hui ; (b) une blessure durable qui se soigne au lit et en convalescence, sans séquelle ; (c) la séquelle possible — un membre estropié, un malus permanent qu'on compense par l'équipement ou une prothèse. Mon avis s'il compte : (b) est le bon premier pas, (c) se décide après l'avoir vu tourner.
2. **L'humeur qui fait agir (axe G).** Aujourd'hui `humeur` ne fait qu'une chose : multiplier la production entre 0,4 et 1,2. Dans RimWorld, un colon malheureux **casse quelque chose** — et c'est la moitié de l'intérêt du jeu. **La question** : un résident malheureux peut-il partir, voler dans les stocks, refuser son poste, se battre avec un autre ? C'est ce qui rend une base vivante, et c'est aussi ce qui la rend ingouvernable. Un seuil trop bas et le joueur ne fait plus que gérer des humeurs.
3. **Le débit (axe J) : RimWorld ou Factorio ?** Aujourd'hui le joueur **assigne des gens** à un périmètre et lit un rendement hebdomadaire ([[Décision — Gestion de base, périmètres de récolte]]). Factorio, c'est autre chose : on **conçoit une chaîne** — un atelier qui consomme à un rythme, un stock tampon, un goulot qu'on trouve et qu'on desserre. Les deux se défendent, et le choix décide de ce qu'est l'endgame de territoire : administrer, ou construire une machine. Je ne peux pas le deviner : il n'y a rien dans le coffre qui penche d'un côté.
4. **La pré-histoire (axe F).** Le monde commence à l'an 1 020 et rien ne s'est passé avant. **Combien d'années lui donne-t-on** — cent, trois cents, mille ? Et le joueur les lit-il **toutes** dans un écran de chronique, ou seulement ce qu'un PNJ veut bien lui dire, par paliers de relation ([[L'information comme récompense]]) ? La deuxième option coûte le même prix et donne une raison de parler aux gens.
