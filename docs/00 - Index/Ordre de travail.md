...
42 bis. **Les 57 autres matériaux « de biome » qu'aucune table ne pose** — levés par le même correctif, en onze catégories (roches, minéraux, métaux, fossiles, liquides…). **Borné aux bois dans l'audit tant que ce n'est pas trié**, parce qu'une bonne part sont sans doute de faux positifs : l'audit ne lit pas encore le sous-sol (`sous_sol.json`), ni toutes les formes de `minerais_par_etage`. D'abord lui apprendre ces sources, **puis** trier ce qui reste — plus `meteorite_ferreuse`, la seule famille de composant sans source.
