...
50. **CE QUI SORT DES DÉCISIONS DU 2026-09-14** *(le designer : « Prends les décisions toi même » — chaque réponse est dans [[Décisions en attente]], révisable d'un mot)*. Dans l'ordre où je les prends — ce qui débloque le plus d'abord, puis ce qui se voit, puis le contenu :
    ...
    · **g. 26 quater — un étage ne montre que son niveau** — **LOT 1 FAIT le 2026-09-14** : à l'étage, le terrain du sol, ses bords mémorisés, les êtres et les plantes du dessous passent **dans l'ombre** (`styles.json` → `etage_vue` : la moitié de la lumière), et seul le plancher du niveau du joueur se dessine en pleine lumière ; **deux défauts levés au passage** — `_dessiner_etage` dessinait l'AIR de l'étage comme un sol d'herbe (les bandes vertes suspendues des captures) et les tuiles du rectangle que le plan de l'étage ne pose pas. Vérifié sur capture (`--ville --graine 7 --a-l-etage`), avant et après. *Reste* : le niveau du joueur en entier (les autres bâtiments à cette hauteur), le sol montré **seulement** sous l'air plutôt qu'assombri partout, et quelques bords de terrain qui gardent leur couleur (dessinés par un autre nœud).
    · ~~**o. Un point hors de sa case**~~ — **FAIT le 2026-09-18** (question 37) : le rig déclare une **marge de case**, et elle se **déduit** de ses ancrages — aucun nombre à la main. Les épaules d'un torse de 9 sont à ±5 : sa case s'élargit d'une unité de chaque côté, le calque de points ne perd plus rien (le générateur signalait deux marqueurs « hors de la case » à chaque passage, il n'en signale plus aucun) et le pantin lit la même marge. *Corrigé en route* : une première version comptait aussi le débord en longueur, alors qu'un ancrage s'y mesure depuis la base — elle donnait des marges de 3 à 5 et aurait doublé la taille des membres. Vérifié sur la capture des pantins. **Et un piège levé (2026-09-19)** : le cou du 2026-09-13 avait été écrit à la main dans les trois JSON de rig, pas dans `tools/gen_rigs.py` — relancer le générateur l'effaçait (deux tests rouges, une erreur de script). Le cou est maintenant DANS le générateur, qui reproduit les rigs publiés à l'octet près, la marge en plus.
    · ~~**n. Une tête qui porte déjà son visage**~~ — **FAIT le 2026-09-14** (question 40 ; `styles.json` → `planches.tetes_traits_inclus`) : la tête `ovale` dessinée par le designer porte ses yeux, son nez et sa bouche ; le jeu ne les redessine plus par-dessus (vérifié sur la galerie des têtes : plus de traits doublés) ; oreilles, cheveux et pilosité restent posés. *Constaté au passage* : les six têtes « dessinées » ont toutes leur calque de points — la question 28 était déjà réglée.
    · **h. L'interface** — **en partie FAITE le 2026-09-14** : le pavé d'informations du prototype est **caché par défaut** et revient par **F3** (remappable, `controles.json` → `infos`) — le HUD porte déjà les barres et la hotbar ; le zoom plafonne à **6** (`styles.json`) ; **l'armure ne peignait déjà que ses zones** (`slots_segments` du rig : la cuirasse couvre torse et bassin) — le corps uni venait du kit de départ, qui couvre presque tout. ~~*Reste* : l'ombrage de la tête~~ — **FAIT le 2026-09-18** (question 38 ; `styles.json` → `planches.ombrage_code` = 0,78) : la tête et les segments que le code dessine encore (le cou) prenaient la couleur de peau en plein, quand les sprites du corps sont en gris moyen — la tête sortait plus claire que le corps qu'elle surmonte. Les deux passent par le même facteur ; vérifié sur la galerie des têtes avant/après.
