...
|---|---|---|---|
| **bruit** | Le **bruit de Perlin** — `FastNoiseLite`, les huit couches de `noise_layers.json`, `monde.surface.bruits`. Le mot est partout dans la génération. | Le **champ sonore** à venir (ce qui s'entend : une porte, un combat, une forge, un éboulement). | **Le bruit de Perlin garde le mot.** Le champ sonore en cherche un autre — *son*, *rumeur sonore*, *vacarme* ? **La question est ouverte et bloque la première ligne de ce champ** ([[Décisions en attente]], n° 9). |
