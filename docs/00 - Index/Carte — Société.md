---
aliases: ["7", "7. Systèmes de vie simulée", "Carte — Société", "Carte Société"]
tags: [index, carte]
domaine: index
statut: décidé
etape: 10
---

De la relation individuelle au royaume : commerce, réputation, guildes, territoire. 38 notes.

*Les cinq systèmes annoncés en tête de la section 7 du GDD :*
- Quêtes de guilde
- Réputation et relations PNJ
- Commerce et boutiques (achat/vente, tenir sa propre boutique)
- Agriculture et élevage
- Apprivoisement de créatures/montures

**Le commerce**
- **[[Commerce et boutiques]]** — vendre aux marchands, ou tenir sa boutique passive.
- **[[Boutique passive]]** — les clients viennent tout seuls.
- **[[Prix suggéré]]** — la formule, et la tolérance au sur-prix.
- **[[Économie — sources et puits]]** — *l'or doit pouvoir disparaître du jeu, pas seulement circuler.* La règle unifiée du portefeuille fini.
- **[[Barèmes économiques]]** — les chiffres.

**Les relations**
- **[[Réputation et relations]]** — quatre niveaux en parallèle, conséquences par palier.
- **[[L'information comme récompense]]** — *l'information est la récompense principale de la relation, avant les prix.* Deux axes par paliers.
- **[[Voie de rédemption]]** — *un joueur ne peut jamais être socialement muré.*
- **[[Quêtes et guildes]]** — 12 guildes, quêtes procédurales, 5 rangs.
- **[[Gabarit de quête]]**

**L'élevage — un jeu dans le jeu** *(Annexe H)*
- **[[Élevage — intention et familles]]** — attraper, croiser, compléter. Six familles, un verbe chacune. *Aucun `if (espèce === 'x')`.*
- **[[Règle d'anneau]]** — le mécanisme d'hérédité : 34/34/16/16. *Une valeur lointaine ne s'obtient jamais d'un coup, elle se marche.*
- **[[Loci — les dix types]]** — dix mécaniques de transmission, une fonction chacune. Pas de plafond.
- **[[Conditions de reproduction]]** — quinze prédicats, six coûts, et la règle : **afficher la raison, jamais un bouton grisé muet.**

**La vie simulée**
- **[[Agriculture et élevage]]** — et le principe transversal d'abstraction hors-site.
- **[[Faim]]** · **[[Faim des PNJ]]**
- **[[Cuisine et alchimie]]** — cuisiner pour la croissance long terme, distiller pour la puissance court terme.
- **[[Nourriture, potentiel et potions]]**
- **[[Habitat des PNJ]]** — *l'humeur est LE levier de rendement.*
- **[[Détection de pièces]]** — trois usages depuis un seul algorithme.
- **[[Abstraction hors-site]]** — résolution par formules, jamais de simulation accélérée.

**Le royaume**
- **[[Royaume du joueur]]** — la destination naturelle d'une partie. Six chantiers.
- **[[Expansion territoriale]]** · **[[Population et exploitation]]** · **[[Halls de guilde]]**
- **[[Villes — population, quartiers et économie]]** — la ville à la population : quartiers sur plusieurs cellules, rues, logements, commerces, production, champs et bêtes, économie, siège du pouvoir.
- **[[Royaumes — état, ères, blasons et événements]]** — le programme D : un pays simulé chaque semaine, son règne et son ère, son blason sur ses gardes, ses événements.
- **[[Un monde réel — villes, PNJ, royaumes et calendrier]]** — le programme du 5 septembre : villes à la population, PNJ distincts, royaumes-pays, calendrier, agriculture ; puis la 0.5.0.
- **[[Gouvernance, lois et diplomatie]]** — six gouvernances, des lois propres à chaque royaume (dont des lois absurdes assumées), quatre accords diplomatiques.
- **[[Lois et infractions]]** — *l'infraction n'a de conséquence QUE si repérée.* Pas de karma caché.
- **[[Génération des royaumes PNJ]]** — des îlots de civilisation. *Un royaume est un événement.*
- **[[Schéma royaume]]** · **[[Défense et raids]]** · **[[Raids et menaces]]** · **[[Entretien et taxes]]**

**Le multijoueur**
- **[[Multijoueur]]** — coop 4-8, host-and-join, PvP en duel consenti uniquement.

## Liens
- **Voir aussi** : [[Sensen — Index général]], [[Carte — Êtres]], [[Carte — Monde]], [[Carte des dépendances]]
