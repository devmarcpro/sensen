...
| **Les armes coûtent de la vigueur (ex-« endurance »), les sorts du mana** | Les 17 modules de manuel passent de `cout_mana` à `cout_endurance` : *Le Sabre* devient jouable sans Volonté, l'endurance obtient enfin son rôle, et « attendre » (5 ticks, rend 20 d'endurance) devient une décision de combat. Les 61 modules sont transcrits dans le schéma avec un `cout_ticks` ([[Modules]], [[Décision — Transcription du catalogue de modules]]) |
