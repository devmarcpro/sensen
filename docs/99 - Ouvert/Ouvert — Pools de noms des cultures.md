...
**Le volume :** *quelques dizaines d'entrées par pool* suffisent — l'algorithme de concaténation A+B ([[Génération de noms]]) produit une variété énorme depuis peu de matière.
...
> `culture_sino` est renommé `sino` ; `famille_b` et `prenom_b` sont remplacés par des pools dédoublés par genre (`famille_b_m` / `famille_b_f`, `prenom_b_m` / `prenom_b_f`).
...
- **Voir aussi** : [[Races]], [[Localisation]], [[Décisions fondatrices]]
