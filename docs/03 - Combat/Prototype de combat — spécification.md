...
> **Jalon 8 codé** : `systems/combat/capacites.gd` (assemblage d'une séquence → plan : coûts en ticks et dans la monnaie du noyau, dés, forme, conditions, drapeaux ; géométrie des formes), intentions `capacite` (F1-F3 puis clic, forme prévisualisée en bleu), mana avec surchauffe, friendly fire des zones, segment de chaîne par capacité qui touche, résolveur non offensif ×0.7 sur le soin. Infobulle exhaustive de chaque capacité (forme, portée, coûts, dés calculés).
> ⚠️ **Conflit signalé** : la spec ci-dessus parle de « 17 modules » (les 5 de Feu, 5 d'Eau, 4 de Foudre, 5 de Métal de l'ancien catalogue à 61) alors que [[Modules]] a été refondu le même jour en **176 composants** où forme et effet sont séparés. La note la plus récente fait foi : le prototype puise dans les 176. Avec **3 compétences × 3 modules**, l'aventurier porte trois capacités déclarées dans `creatures/aventurier.json` (`capacites`) : *Étincelle* `[point, etincelle]`, *Gel en ligne* `[ligne, gel, concentration]`, *Baume* `[soi, baume]` — Feu et Eau par la magie, Métal/Terre/Bois par le râtelier : la rotation complète reste possible. **Ré-assembler = éditer le JSON + F5** (pas d'écran d'assemblage dans le prototype).
> **Jalons 9 à 12 codés** : projectiles (trajectoire réelle, allié qui masque = tir refusé, ennemi interposé = touché, munitions, récupération 50 %), statuts en données avec anti-stunlock et interruption des jauges (Étourdi), statuts par actions de créatures et par modules (Feinte, Entrave, Égide, Saignement, tempo : Étourdissement, Torpeur, Célérité, Rapt), profil IA `elite` qui construit sa chaîne, écran de fin (issue, durée en ticks, XP des quatre pistes).
> **2026-08-27** : les six types de modules sont désormais tous résolus au moins en partie — déclencheurs *À l'impact* / *Curée* et liaisons *Répétition* / *Ricochet* / *Dispersion* / *Miroir* / *Partage* ([[Modules]], callout du 2026-08-27).
> **2026-08-27 (suite)** : glyphes (Sceau), Mèche, Écho, Barrière, Exhaussement, Fosse, Racine — la *Ruine à estrades* peut désormais jouer ses glyphes.
> **2026-08-27 (fin)** : déclencheurs à événement (Riposte, Parade, Ouverture, Veille, Testament, Accord, Cadence) et liaisons Salve / Propagation / Boucle / Contagion — le catalogue des 176 composants est résolu à l'exception de Dérobade, Alternance, Meute et des noyaux de saisie/lévitation/portails (mécaniques d'étapes ultérieures).
> **Ce qui reste hors du code, et pourquoi** : l'*embuscade* (+2 dés si non détecté) et *semer en Discrétion* exigent que les créatures aient une notion de détection du joueur et que la compétence Discrétion existe — étape 4 (compétences) ; l'*Esquive* comme mobilité (−0,5 %/niveau) attend aussi les niveaux. Le **polissage UI** est un jugement d'œil : voir [[Vers la production]] pour les questions à trancher.

> [!success] Codé le 2026-08-29 — l'embuscade et semer en Discrétion, les derniers « hors du code » de cette note
> *Semer* : voir [[IA des créatures]] (callout du même jour) — l'acquisition et le suivi de cible passent par la détection complète, une cible perdue de vue 100 ticks est lâchée.
> Trois créatures (lynx, crocodile, serpent venimeux) portent l'action passive `embuscade` (`bonus_premiere_attaque: 2 dés`) et le code la laissait passer (« la détection du joueur n'existe pas encore »). Elle existe : la **première frappe** d'une créature sur une cible **qui n'est pas encore en combat** — la frappe qui ouvre le combat — reçoit les dés. **Décision** : « non détecté » se lit *la proie ne se bat pas encore*, pas *la proie ne voit pas l'attaquant* — à portée de griffe (1 tuile) le cône de détection voit toujours, la surprise est un état de combat, pas de vue ; elle ne se rejoue pas contre la même proie (`surprise_sur` consommé à la frappe). Journal `journal.embuscade`. Le joueur en forme bestiale profite du même mécanisme si ses actions de Lune en portent une (aucune aujourd'hui).

> [!success] Tranché le 2026-09-02 — **le premier étage est peuplé de très bas niveau** (designer)
> « Premier étage : ennemis très bas niveau. » Le mur du premier étage était mesuré depuis des jours : aucun profil de robot ne dépassait l'étage 2, même équipé — 1 à 2 dégâts placés par coup contre 9 à 17 encaissés, 87 coups pour tuer un simple guérisseur bandit. Sur les trois leviers proposés — baisser l'armure des hostiles, donner un objet assemblé au départ, ou assumer le mur — le designer a choisi le premier, et sous sa forme la plus nette : ce ne sont pas les armures qu'on rabote, c'est **le niveau des êtres du premier étage** qui descend très bas.
> **Comment c'est fait** : le pool d'un thème va du rat géant au troll, et le premier étage y puisait comme le dixième — on pouvait tomber sur un **lindworm au premier pas**. Chaque étage a maintenant un **plafond de puissance** (`combat_rules.peuplement_etage`), et la puissance d'un être se lit sur sa fiche : force + endurance + moitié de la dextérité, plus un bonus par action spéciale — un troll ne fait pas que frapper fort, il a des tours dans son sac. Si aucun être ne passe le plafond, le plus faible du thème sort quand même : un étage vide n'est pas un étage.
> **Mesuré** (ruine) : étage 1, cinq types faibles ; étage 2, onze ; étage 3, treize ; étage 5, dix-sept — le troll et le lindworm n'apparaissent qu'à partir du cinquième. Le robot équipé passe de l'étage 2 à l'étage 3 et de 16 à **30 kills**.
> **Ce que ça ne règle pas, et qu'il faut dire** : il meurt encore trois fois. Mais il encaisse désormais **2 dégâts par coup** — il ne se fait plus écraser, il se fait **submerger**, et il ne se soigne jamais (soins 0 sur tous les parcours). La part qui reste tient autant à la façon de jouer du robot qu'à l'équilibrage.
> **Pourquoi c'est le bon choix** : les deux autres leviers déplaçaient le problème (un objet de départ rend le mur franchissable une fois, puis il revient au deuxième donjon) ou l'assumaient sans le résoudre. Ici, la difficulté redevient une **pente** : on entre dans un donjon par des créatures faibles, et c'est la descente qui mord.
