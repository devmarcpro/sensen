...
> [!important] 2026-09-05, 13 h 30 — le designer : « retravaille les spawns en donjon des ennemis et du loot, retravaille les ratios pour les premiers niveaux, actuellement c'est trop lent de progresser au début »
> Ce que le peuplement faisait : une créature par 30 tuiles de salle, × (1 + 0,25 × étage), sous un plafond de puissance par étage. Une salle moyenne (≈ 90 tuiles) en tirait trois au premier étage, une grande (≈ 225) neuf, une **immense** (≈ 530) **vingt-deux** — et le robot au kit de départ passait tout son budget dans un seul combat. **Décision** : un **plafond par salle**, `peuplement_etage.max_par_salle_base` (2) + `max_par_salle_par_etage` (1) × (étage − 1) — deux créatures par salle au premier étage, trois au deuxième, quatre au troisième — la densité par tuile reste, elle ne peut plus dépasser ce plafond. Le loot ne change pas de densité (un coffre par 18 tuiles et un objet par ennemi, « le loot pour tous » du 31 août) : ce sont les niveaux qui progressaient trop lentement, pas les objets qui manquaient — voir [[Progression par l'usage]]. Mesuré par le robot avant et après (ligne « progression à l'étage N »). Ce qui reste au designer est dans [[À juger — parcours de jeu]].
