...
> [!important] Décidé le 2026-09-07, 17 h — on frappe des veines en creusant : une mine sans minerai n'est qu'une carrière
> Mesuré en lisant les données : les murs d'une mine ne prennent que des **roches** (pierre, brèche volcanique, andésite, basalte, granit, granit noir selon la bande de profondeur), et les poches de strates ne font que remplacer une roche par la roche voisine. Autrement dit, on peut creuser jusqu'à l'étage 999 sans jamais trouver **un gramme de cuivre**. Le fichier s'appelle pourtant `minerais_par_etage.json` et porte trois paliers de minerais et de gemmes — ils ne servaient qu'aux filons de surface et aux donjons.
> **Une veine est une tuile dont le mur est du minerai** : rien de neuf à inventer, on creuse déjà un mur et l'on en récolte la matière. Un bruit dédié par étage sème des veines (`planete.mine.veines` : la fréquence, le seuil, la part) ; le **palier du minerai suit la profondeur** (les bandes de `tier_par_profondeur`) — cuivre et étain près du jour, fer et houille plus bas, or et gemmes au fond. Le tirage est déterministe (graine, mine, étage) : deux joueurs de la même partie trouvent la même veine au même endroit, et une galerie rouverte retrouve la sienne.
> **Ce qui rend cela juste** : la roche du fond est aussi la plus dure à percer (basalte 48, granit noir davantage) et les paliers hauts n'existent qu'en bas. La récompense se paie en dureté, pas en chance.
> **La récompense atterrit, et l'outil la garde** : `test_veines_de_mine` creuse une veine de cuivre à la pioche et vérifie que le métal entre au sac. Ce qui joue alors, sans qu'on ait rien ajouté, c'est la barrière des **paliers de matériau** (Récolte) : `outil_min` relève le seuil de dureté d'outil exigé et `extraction_ticks` allonge le temps. Une veine d'or au fond ne se ramasse donc **pas** à la pioche de départ — on la voit, on ne la prend pas encore. C'est exactement la tension qu'on veut d'une mine : le fond est riche, et il attend de meilleurs outils.
> **À juger** (les nombres, pas la règle) : la fréquence et le seuil des veines — combien de tuiles sur cent doivent être du minerai pour qu'un puits vaille la peine sans que la mine devienne un self-service.
