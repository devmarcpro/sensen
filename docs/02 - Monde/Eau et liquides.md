...
> [!success] Codé le 2026-08-31 — le chemin sait que l'eau refuse la surcharge
> La sonde de parcours (graine 73, 6 objets portés) piétinait à l'étage 1 : 21 pas puis 569 attentes — `chemin()` (A*) proposait d'entrer dans l'eau, que `_deplacer` refuse à un être en surcharge (« le poids tire vers le fond ») ; le robot, l'IA (`_ia_pas_vers`, la routine, la fuite) et l'aperçu du joueur (tuiles atteignables, chemin cliqué) se cognaient en boucle contre le bord. `Grille.cout_pas`/`chemin`/`atteignables` gagnent `eviter_nage` : l'entrée terre → eau vaut −1 (eau ↔ eau reste libre, la glace ne compte pas) ; `Simulation.refuse_nage(e)` (surchargé, non volant, règle `nage.refus_surcharge`) le fournit à tous les appels. Le pas adjacent volontaire du joueur reste permis : le journal « coule » répond, larguer des objets reste le geste.

> [!success] Codé le 2026-08-31 — le butin ne sèche plus la rivière
> Suite du correctif « la tuile paraissait sèche » du courant : `niveau_liquide` retombait sur le niveau mémorisé, mais les **règles de nage** (coût de pas, refus de surcharge, `dans_l_eau`) lisaient encore les tags du **contenu** — poser du butin sur une source d'eau rendait la tuile marchable et sèche. `Grille.poser_contenu` mémorise désormais le niveau d'une tuile liquide au moment où son contenu est remplacé, et le nouveau `Grille.nageable(p)` (tag `nage` OU niveau mémorisé > 0, hors gel) porte toutes les règles : nager coûte pareil au-dessus d'un tas de butin, la surcharge y refuse toujours l'entrée, et l'A* le sait.
