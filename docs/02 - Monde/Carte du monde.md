...
> [!decision] 2026-09-06, 8 h — la carte gardée en cache (designer : « la carte du monde lag énormément, impossible de se déplacer dessus, il faudrait la garder en cache totalement comme ça on pourrait ouvrir fermer, zoomer dézoomer, se déplacer à notre guise »)
> La carte redessinait chaque cellule visible à chaque image et à chaque mouvement de souris : résumé de cellule, royaume, danger, donjons, gouffre, vingt-cinq sous-points — deux mille cellules, cinquante mille rectangles. Désormais **le monde est une image** : une texture de base d'un pixel par cellule pour le monde entier (1 024 × 512), peinte une fois par tranches en arrière-plan (les tranches autour du joueur d'abord), et des **tuiles de détail** de 64 × 64 cellules à cinq sous-points par côté, peintes à la demande quand on zoome dessus, gardées pour la partie (les altitudes le sont déjà dans la sauvegarde). Par-dessus, un **voile d'état** (cellules non explorées assombries, danger) refait à l'ouverture depuis les chunks explorés, et les repères dynamiques seulement pour les cellules connues (donjons de corruption, claims, capitales, noms, routes, joueur, survol). **Le zoom revient** (molette, de 3 à 40 pixels par cellule ; la texture de base suffit de loin, le détail s'affiche dès `sous_points_min_px`), le glisser et les flèches déplacent la vue sans rien recalculer. Ouvrir, fermer, zoomer, se déplacer : un dessin de trois textures et quelques repères.
>
