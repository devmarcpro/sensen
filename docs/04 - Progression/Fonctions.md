...
> [!important] 2026-09-04, 16 h — les pools ne suivaient pas la table, et les classes cachées n'étaient pas rares
> Relu contre les données : `classes_possibles` mélangeait des classes cachées aux visibles dans les pools (l'aventurier n'avait **que** des cachées, l'artisan trois sur sept), donc un artisan sur deux était un Engrenage ou un Sceau — la note dit « ≈ 2 % », tirées **avant** le pool. Et quatre fonctions du catalogue n'existaient pas en données (éleveur, cuisinier, couturier, transporteur). Désormais : les pools sont **la table ci-dessus, telle quelle** (prêtre tire La Paume ; maître de guilde, le pool du dirigeant) ; les onze classes cachées portent `cachee: true` dans `data/classes/` ; `_habiller_pnj` tire d'abord une cachée à `combat_rules.pnj.classe_cachee_chance` (0,02) parmi toutes, sinon dans le pool. Les quatre fonctions manquantes sont en données (compétences élevage, cuisine, tissage ; le transporteur n'en a pas, comme l'aventurier), sans production hors-site chiffrée pour l'éleveur et le transporteur — leur rendement viendra des systèmes qu'ils servent. La pondération de population de la note n'est pas en données : un hameau pose des **fiches de créatures** par bâtiment (`villageois` → oisif, `fermier`, `forgeron` → artisan, `marchand` → commerçant, `garde_village` → garde, `pretre`, `maitre_de_guilde`, le dirigeant), et c'est la fiche qui porte la fonction — voir [[Villages PNJ — repeuplement et décimation]]. Les quatre nouvelles fonctions n'ont donc pas de fiche de créature ni de bâtiment : elles s'assignent à la base, elles ne naissent pas encore dans les hameaux.
