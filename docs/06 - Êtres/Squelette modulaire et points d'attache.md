...
> [!success] Codé le 2026-09-09 — **quatre directions par personnage** (designer : « on va faire que 4 directions par personnages finalement »)
> **C'est la table du rig qui décide, pas une constante.** Le corps se cale sur l'orientation la plus proche parmi celles que le rig **déclare** : quatre aujourd'hui — la face, le dos et les deux profils, ce qu'un artiste dessine à la main. En rajouter quatre (SE, NE, NW, SW) rendrait les huit angles sans toucher une ligne de code.
...
> **LA CONSÉQUENCE, ET ELLE COMPTE POUR CELUI QUI DESSINE** : sur les huit directions de marche possibles, **six montrent un profil**. La face ne sort que lorsque le personnage descend droit vers le bas de l'écran, le dos que lorsqu'il monte droit vers le haut. Ce sont donc les **deux profils** qui méritent le plus de soin. `capture.tscn -- --pantins [rig]` dessine les huit directions de marche calées sur les vues déclarées : deux colonnes identiques disent que deux directions partagent une vue.
