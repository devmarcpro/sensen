...
- `prenom_a`, `prenom_b_m`, `prenom_b_f`, `famille_a`, `famille_b_m`, `famille_b_f`, `ville_a/b` : pools de la partie A et B, concaténées ([[Génération de noms]]). **Les terminaisons de prénom sont genrées** ; ~~celles de patronyme le sont là où la langue l'exige~~ — **annulé le 2026-09-07** (designer : « un nom de famille n'a pas de sexe ») : `famille_b_m` sert à tous, `famille_b_f` n'est plus lu — voir [[Pools de noms des cultures]]. La partie A est commune aux deux genres. `famille_b: [""]` (chaîne vide) = culture à noms de famille "pleins" plutôt que composés (comme le sino ci-dessus) ; la plupart des autres cultures ont un vrai suffixe.
