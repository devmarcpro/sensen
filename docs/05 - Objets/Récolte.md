...
> [!success] Décidé le 2026-09-01 — les verbes du monde reviennent, l'outil en main (designer, point 52)
> Le retrait de la casse de blocs (point 46) laissait le joueur sans prise sur le décor ; le designer la rend, mais **sous condition d'outil** : **creuser** un mur demande une **pioche**, **terrasser** une tuile une **pelle**, **abattre** un arbre une **hache**, **cueillir** une **faucille**, **inonder** un **seau**, **brûler** une **torche**. Sans l'outil en main, l'option n'apparaît pas — le menu reste court, et l'outil redevient un choix d'équipement (une main occupée par une pioche n'est pas une main qui tient une épée). La table `combat_rules.outils_verbes` la déclare, aucun verbe n'est écrit en dur.
>
> Deux règles de survie l'accompagnent, dans l'esprit de *Rogue*. **La faim tue** : à jauge vide, la famine ronge les PV par paliers (`faim.degats_par_palier` tous les `periode_zero` ticks — la note disait `ticks_par_palier`, que le code ne lit pas, corrigé le 2026-09-07) — la nourriture redevient l'horloge qui pousse à avancer. Et les **objets ne sont plus tous identifiés** : consommables et gemmes sortent du loot **inconnus** (`loot_rules.identification`), sous un nom d'apparence ; les essayer révèle la base **pour toute la partie**, comme les potions de Rogue. Le pari revient dans le jeu.
