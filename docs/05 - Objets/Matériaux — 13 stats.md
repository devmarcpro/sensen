...
> [!success] Codé le 2026-09-09 — **la quinzième et dernière colonne : `permeabilite`**, et l'eau qui s'infiltre (ordre de travail 32)
> **LES CINQ COLONNES DEMANDÉES SONT ÉCRITES.** `fusion`, `portance`, `absorption`, `alteration` et maintenant `permeabilite` : chacune a son champ, et aucune n'attend plus.
> **Celle-ci se dérive autrement, et il faut dire pourquoi.** Les quatre précédentes se déduisaient de stats existantes ; celle-là ne le peut pas — **la perméabilité est affaire de GRAIN, pas de densité**. L'argile et le sable ont, dans nos données, la même densité (10 et 9) et la même dureté dérisoire ; pourtant l'une retient un étang et l'autre le vide en une heure. **Aucune stat du dépôt ne sépare un grain fin d'un grain grossier.** Le socle par catégorie fait donc le gros du travail, et une trentaine de matières **dont la perméabilité est le trait définissant** portent leur valeur en clair : argile 3, limon 22, sable 92, gravier 96, granit 4, béton 4, grès 55, ponce 74. *Ce n'est pas du code en dur : c'est du contenu, et le contenu ici c'est le monde réel.*
> **CE QU'ELLE CHANGE TOUT DE SUITE** : un creux était jusqu'ici un **bassin parfait quel que soit son fond** — on tenait un étang sur du sable, et un trou creusé dans le gravier gardait sa pluie pour l'éternité. Le fond décide maintenant : au-delà de `eau.infiltration_seuil`, la flaque perd un niveau par pas d'automate jusqu'à disparaître. **Et creuser un bassin devient un ouvrage** : il faut la bonne matière au fond, ou l'y poser — le geste existait déjà, il prend un sens.
> **Ce qui reste de la ligne 32** : la **pression** et l'**érosion**. Une nappe sous pression qui jaillit quand on la perce, et une eau qui use ce qu'elle traverse, demandent que l'automate porte une notion de charge — il ne connaît aujourd'hui qu'un niveau par tuile.
