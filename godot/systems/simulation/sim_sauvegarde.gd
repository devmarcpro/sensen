...
static func sauvegarder(sim: Simulation, nom: String = "", en_fond: bool = false) -> bool:
	...
	var monde_json := {"version": int(GameData.config("combat_rules").get("sauvegarde", {}).get("version", 2)), "resume": resume_partie(sim), "graine": sim.graine, "graine_monde": sim.graine_monde, "planete_options": sim.planete_options, "identifies": sim.identifies, "ticks": sim.horloge_monde.ticks, "prochain_donjon": sim.prochain_donjon, "n_entites": sim._n_entites,
		...
		"nettoyages": sim.monde.nettoyages, "lieux_connus": sim.monde.lieux_connus, "ecologie": sim.monde.ecologie, "lieux_nes": sim.monde.surface.lieux().nes_serialise(),
...
static func charger_sauvegarde(sim: Simulation, nom: String = "") -> bool:
	...
	sim.monde.soupcons = w.get("soupcons", {})   # les soupçons d'une ville sur le joueur (question 36)
	sim.monde.faits = w.get("faits", [])   # ce que le monde se raconte survit au rechargement (rumeur, 29)
	...
	sim.monde.lieux_connus = w.get("lieux_connus", {})
	sim.monde.surface.lieux().nes_charger(w.get("lieux_nes", {}))   # les tanières nées de la simulation (39 ter, pas F)
	sim.monde.ecologie = w.get("ecologie", {})   # les lieux découverts en chemin (39 ter, pas E)
